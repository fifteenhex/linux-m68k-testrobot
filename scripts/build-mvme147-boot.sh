...
#                         holding u-boot.img; the SPL loads that to start
#                         full U-Boot (BOOT_DEVICE_SATA, partition 1,
#                         CONFIG_SPL_FS_LOAD_PAYLOAD_NAME=u-boot.img)
#
# Usage: scripts/build-mvme147-boot.sh
#
# Requires the SPL and u-boot.img under output/u-boot-m68k-testrobot/
# (scripts/build-uboot.sh), python3, and the disk tools mkfs.fat
...
# sector 2048, holding u-boot.img in its root.  Built with mtools/mkfs.fat
# so no loop device or root is needed.
